...
//Notes:
//to connect with flask server
//1) give permission internet in manifest

//2 is done to get rid of error  java.net.UnknownServiceException: CLEARTEXT communication to 192.168.100.5 not permitted by network security policy
//2) either write android:usesCleartextTraffic="true" in applicaiton tag of manifest
//   or make a network_security_config.xml file in res/xml/ and give your server ip there, then add
...
//also in emulator settings change proxy to manual proxy configuration and add your server ip(or 10.0.2.2) and port
//add new access point in emulator give ip 10.0.2.2 and port, name, APN and then connect to that access point
